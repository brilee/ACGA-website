...
def score_likely_games(kgs_game, KGS_name1, KGS_name2, date=None):
    ...
    if date is not None:
        if abs(date - kgs_game.date) > timedelta(days=7):
            score -= 5
    if kgs_game.black.startswith(KGS_name1) and kgs_game.white.startswith(KGS_name2):
        score += 5
    if kgs_game.black.startswith(KGS_name2) and kgs_game.white.startswith(KGS_name1):
        score += 5
    return score
